    ...
    >
      ...
      >
        <v-card class="elevation-12">
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="primary" :disabled="loading.submitting" @click="handleSubmit()">Update Password</v-btn>
          </v-card-actions>
        </v-card>
      </v-flex>
    </v-layout>
  ...
  methods:{
    async handleSubmit() {
      ...
      try {
        ...
        sharedService.generateSnackbar(this.$root, data.updateForgottenPassword.message, 'success');
      } catch(err) {
