  ...
  >
    ...
    > 
      ...
      <div v-else-if="loading.joiningRoom">
        <span class="display-1 pl-2">Joining Room...
          <v-progress-circular indeterminate></v-progress-circular>
        </span>
      </div>
      <div v-else-if="generation.room < 3 && ($apollo.queries.room.loading || $apollo.queries.rounds.loading)">
        <span class="display-1 pl-2">Loading Room...
          <v-progress-circular indeterminate></v-progress-circular>
        </span>
      </div>
      <div v-else-if="!room">
        <span class="display-1 pl-2">Initializing Room...
          <v-progress-circular indeterminate></v-progress-circular>
        </span>
      </div>
      <div v-else style="width:100%;">
          ...
          <v-row justify="center">
            <v-col cols="12" class="text-center">
              <div v-if="activeRound" class="text-center justify-center">
                <v-layout v-if="settingsObject.showScramblePreview" align-center justify-center>
                  <ScrambleDisplay :puzzle="activeRound.scramble.event && activeRound.scramble.event.puzzle" :scramble="activeRound.scramble.scramble" :solved="!currentUserSolve || (currentUserSolve && currentUserSolve.state == 'FINISHED')" :visualization="settingsObject.scramblePreviewVisualization"></ScrambleDisplay>
                </v-layout>
                <div>
                  <div v-if="loading.updatingSolve">
                    <span class="display-1 pl-2">Submitting Results...
                      <v-progress-circular indeterminate></v-progress-circular>
                    </span>
                  </div>
                  <span v-else-if="!currentUserSolve || (currentUserSolve && currentUserSolve.state == 'FINISHED')" id="v-step-scramble" class="display-1">Waiting for next round...</span>
                  <div v-else :class="{ 'grey--text': chatBoxFocused }">
                    <Scramble :scramble="activeRound.scramble" :size="settingsObject.scrambleFontSize" id="v-step-scramble" />
                  </div>
                </div>
              </div>
            </v-col>
            <v-col xl="9" lg="12">
              <v-card
                ...
                tile
              >
                <v-simple-table fixed-header id="roomResults">
                  <template v-slot:default>
                    <thead>
                      <tr>
                        <th>Cuber</th>
                        <th class="text-right" width="150">{{ activeRoundNumber ? "#" + activeRoundNumber : "&nbsp;" }}</th>

                        <th v-for="i in tableRounds-1" :key="i" class="text-right" width="100"><a @click="openViewRoundDialog(rounds[i])">{{ rounds[i] ? "#" + rounds[i].round_number : "" }}</a></th>

                        <th v-for="(accItem, i) in accumulatedResults" :key="accItem.name" width="100" class="text-right" :class="{'td-border-left' : i == 0 }">
                          {{ accItem.name }}
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="item in cuberResults" :key="item.id" :class="{ 'item-highlight': item.updating }">
                        <td>
                          <PreviewCuberPopover :selectedItem="item.cuber">
                          </PreviewCuberPopover>
                          <v-icon v-if="item.id == room.creator.id" small color="yellow" title="Room Creator">mdi-crown</v-icon>
                          <v-icon v-if="room.manager && room.manager.id == item.id" color="green" small title="Room Manager">mdi-gavel</v-icon>
                          <v-icon v-else-if="isRoomManager" small title="Assign as manager" @click="assignNewManager(item.id)">mdi-gavel</v-icon>
                          <v-icon v-if="!item.isActive" small title="Away">mdi-sleep</v-icon>
                        </td>
                        <td v-for="i in tableRounds" :key="i" class="text-right title" v-html="renderSolveResults(rounds[i-1], item.id)"></td>

                        <td v-for="(accItem, i) in accumulatedResults" :key="accItem.name" class="text-right title" :class="{'td-border-left': i==0}">
                          <span @click="openViewAccumulatedResultDialog(item.id, accItem)" class="pointer-cursor" v-html="renderAccumulatedResults(item.id, accItem)"></span>
                        </td>
                      </tr>
                    </tbody>
                  </template>
        ...
        <v-row justify="center">
          <span class="caption grey--text">
            Room Name: {{ room.name }} | Event: {{ room.event.name }} | Time Limit: {{ room.time_limit/1000 || "None" }} | Time Target: {{ room.time_target/1000 || "None" }} | Max Capacity: {{ room.max_capacity || "None" }}
          </span>
        </v-row>
        <v-row justify="center">
          <v-col cols="12" class="text-center">
            <v-btn v-if="isRoomManager" @click="startNextRound()" :loading="this.loading.startingNextRound" id="v-step-startroom">{{ activeRound ? "Start Next Round" : "Start Room" }}</v-btn>
            ...
            -->
            <v-btn @click="copyShareLink()" id="v-step-share">Share Link</v-btn>
            <v-btn @click="openViewRoundsDialog()">All Rounds</v-btn>
            <v-btn @click="startTutorial()">Tutorial</v-btn>
            <v-btn @click="openEditRoomSettingsDialog()" icon id="v-step-settings">
    ...
    >
      <v-expansion-panels v-model="expansionOpenedIndex">
        <v-expansion-panel>
          <v-expansion-panel-header expand-icon="mdi-chevron-up" class="title py-0">
            <span>
          ...
          <v-expansion-panel-content>
            <ChatHistoryCard :chatMessages="chatMessages"></ChatHistoryCard>
            <div class="chat-box">
              <v-text-field
                name="chatbox"
                v-model="chatMessage"
    ...
    <CubeTimerOverlay :disabled="cubeTimerDisabled" :inputMethod="settingsObject.inputMethod" :timerTriggerDuration="settingsObject.timerTriggerDuration" :inspectionTimer="settingsObject.inspectionTimer" @submit-results="updateSolve" @timer-state-updated="handleTimerStateUpdate" @timer-status-updated="handleTimerStatusUpdate" @stopPropagation="stopPropagation = true"></CubeTimerOverlay>
    <ViewRoundDialog :roundId="lookupRoundId" :status="dialogs.viewRound" @close="dialogs.viewRound = false"></ViewRoundDialog>
    <ViewRoundsDialog :room="room" :status="dialogs.viewRounds" @close="dialogs.viewRounds = false"></ViewRoundsDialog>
    <EditRoomSettingsDialog :status="dialogs.editRoomSettings" :settingsObject="settingsObject" @update-settings="updateSettings" @close="dialogs.editRoomSettings = false"></EditRoomSettingsDialog>
    <ViewAccumulatedResultDialog :selectedItem="lookupAccumulated" :status="dialogs.viewAccumulated" @close="dialogs.viewAccumulated = false"></ViewAccumulatedResultDialog>
...
import { JOIN_ROOM_MUTATION, LEAVE_ROOM_MUTATION, GRANT_ROOM_MANAGEMENT_TO_CUBER_MUTATION, NEXT_ROUND_MUTATION, SEND_ROOM_CHAT_MESSAGE_MUTATION, UPDATE_SOLVE_MUTATION } from '~/gql/mutation/room.js'
import { IDLE_MUTATION } from '~/gql/mutation/cuber.js'
import { ROOM_MEMBERS_UPDATED_SUBSCRIPTION, ROOM_UPDATED_SUBSCRIPTION, ROOM_CHAT_MESSAGE_RECEIVED_SUBSCRIPTION, ROUND_STARTED_SUBSCRIPTION, ROUND_FINISHED_SUBSCRIPTION, SOLVE_UPDATED_SUBSCRIPTION } from '~/gql/subscription/room.js'
...
export default {
  ...
  head() {
    return {
      ...
      meta: [
        { hid: 'description', name: 'description', content: 'Cube.Zone is a new online platform for social cubing' }
      ]
    }
  },
    ...
    $subscribe: {
      ...
      roundFinished: {
        ...
        result({ data }) {
          ...
          let foundRound = this.rounds.find(round => round.id == data.roundFinished.id);
      ...
      roundStarted: {
        ...
        result({ data }) {
          ...
          this.cuberResults.length ? this.addCuberResults(data.roundStarted) : this.buildCuberResults();
      ...
      solveUpdated: {
        ...
        result({ data }) {
          if(data.solveUpdated.id in this.solvesMap) {
            this.updateObject(this.solvesMap[data.solveUpdated.id], data.solveUpdated);
          }
          //flash the user time that just got updated
          this.flashResultUpdating(this.cuberResults.find(cuber => cuber.id == data.solveUpdated.cuber.id));
        },
        fetchPolicy: 'no-cache',
        skip: true
      },
  ...
  methods: {
    updateSettings(settingsObject) {
      ...
      if(process.client) {
        localStorage.setItem("roomSettings", JSON.stringify(this.settingsObject));
      }
    ...
    renderSolveResults(round, cuberId) {
      ...
      return "<span" + (foundSolve.is_winner ? " class='winner-text'" : "") + ">" + sharedService.generateSolveString(foundSolve, round.round_number === this.activeRoundNumber) + "</span>";
    },
    ...
    renderAccumulatedResults(cuberId, accItemObject) {
      ...
      return "<span" + (foundResult.is_winner ? " class='winner-text'" : "") + ">" + sharedService.generateAccumulatedResultString(foundResult) + "</span>";
    },

    findAccumulatedResult(cuberId, accItemObject) {
      return this.rounds[1].accumulated_results.find(accumulator => accumulator.contextualAccumulator.type_id === accItemObject.type_id && accumulator.contextualAccumulator.pivot_n === accItemObject.pivot_n && accumulator.cuber.id == cuberId);
    },
    ...
    addActiveCubers(activeCubers, initialLoad) {
      ...
      activeCubers.forEach(active_cuber => {
        let foundResult = this.cuberResults.find(cuber => cuber.id == active_cuber.id);

        if(!foundResult) {
          !initialLoad && this.handleChatMessageReceived({
            user: active_cuber,
    ...
    removeActiveCubers(activeCubers, initialLoad) {
      ...
      this.cuberResults.forEach(cuber => {
        let foundResult = activeCubers.find(active_cuber => active_cuber.id == cuber.id);
    ...
    buildCuberResults() {
      this.rounds.forEach(round => {
        round.solves.forEach(solve => {
          const foundResult = this.cuberResults.find(cuber => cuber.id == solve.cuber.id);
    ...
    addCuberResults(round) {
      ...
      round.solves.forEach(solve => {
        let foundResult = this.cuberResults.find(cuber => cuber.id == solve.cuber.id);
    ...
    findCurrentUserSolve() {
      ...
      const solve = this.activeRound.solves.find(ele => ele.cuber.id == currentUserId);
    ...
    checkCuberResults() {
      for(let i = this.cuberResults.length-1; i>=0; i--) {
        ...
        if(!this.cuberResults[i].recentActiveRound || this.cuberResults[i].recentActiveRound.round_number <= this.activeRoundNumber - this.tableRounds) {
    ...
    async updateSolve(solveResult) {
      ...
      try {
        ...
        if(!this.currentUserSolve) {
          throw sharedService.generateError("Could not find your solve in this round");
        }
    ...
    handleTimerStateUpdate(timerState) {
      //solve must progress
      if(timerState > this.timerState && (timerState == 1 || timerState == 4 || timerState == 5)) {
    ...
    async assignNewManager(cuberId) {
      ...
      try {
        ...
        sharedService.generateSnackbar(this.$root, "Assigned New Manager", "success");
      } catch(err) {
        sharedService.handleError(err, this.$root);
      }
  ...
  computed: {
    cubeTimerDisabled() {
      return !(this.currentUserSolve && this.currentUserSolve.state != 'FINISHED') || this.chatBoxFocused || this.loading.updatingSolve || this.dialogs.editRoomSettings;
    },
