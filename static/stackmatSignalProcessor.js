...
class StackmatSignalProcessor extends AudioWorkletProcessor {
  ...
  processSignal(signal) {
    ...
    if (this.signalIsEdge(signal, lastSignal)) {
      for (
        let i = 0;
        i < Math.round(this.signalDuration / this.bitSampleRate);
        i++
      ) {
  ...
  signalIsEdge(signal, lastSignal) {
    return (
      Math.abs(lastSignal - signal) > THRESHOLD_EDGE &&
      this.signalDuration > this.bitSampleRate * 0.6
    )
  }

  processByteBlock() {
    const state = decodeByteBlock(this.byteBuffer) || {
      ...StackmatStates.get('X'),
    }
    this.byteBuffer = []
...
class BitStream {
  ...
  isFull() {
    if (this.buffer.length >= 10) {
      if (
        this.buffer[0] == this.idleValue ||
        this.buffer[9] != this.idleValue
      ) {
