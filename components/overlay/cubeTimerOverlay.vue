  ...
  > 
    <v-row>
      <v-col cols="12" class="text-center">
        <span v-if="timerState >= 5 && (solveResult.is_dnf || isInspectionDnf)" class="red--text timer-text">DNF</span>
        <span v-else class="timer-text" :class="{'black--text': isInspecting, 'red--text text--lighten-4': timerState == 2, 'green--text text--darken-4': timerState == 3 }">{{ generateTimeString(timeElapsed) }}</span>
        <span v-if="solveResult.penalties" class="red--text timer-text">(+{{ solveResult.penalties/1000 }})</span>
        <div v-if="isInspectionDnf">Your solve was DNFed due to inspection taking longer than 17 seconds.</div>
        <div v-else-if="isInspectionPenalty">Your solve has incurred an extra +2 penalty because the inspection took between 15 and 17 seconds.</div>
      </v-col>
    </v-row>
    <v-row>
      ...
      <v-col cols="12" class="text-center">
        <div v-if="inputMethod == 'manual'" class="overline">Manual Entry - Please Type in the Result using your Keyboard</div>
        <div v-if="inputMethod == 'stackmat' && timerState == 5" class="overline">Don't forget to reset your Stackmat Timer!</div>
      </v-col>
    </v-row>
    <v-row>
      <v-col cols="12" class="text-center" style="height:36px">
        <template v-if="timerState == 5 || timerState == 6">
          ...
          <v-btn color="success" :loading="timerState == 6" @click="timerState = 7">
            Finalize Results (or hold Space)
          </v-btn>
        </template>
        <v-btn v-else-if="timerState == 8" color="warning" @click="timerState = 9">Go Back</v-btn>
      </v-col>
    </v-row>
  </v-overlay>
</template>
...
export default {
  ...
  created() {
    if(process.client) {
      ...
      this.stackmat.on('started', (packet) => {
        //not allowed to skip inpsection if there is one
        if(this.timerState !== 4 && (!this.inspectionTimer || this.timerState === 1)) {
          this.timerState = 4;
        }
      });
  ...
  methods: {
    ...
    togglePenalty() {
      if(this.isInspectionPenalty) {
        this.solveResult.penalties == 4000 ? this.solveResult.penalties -= 2000 : this.solveResult.penalties += 2000;
      } else {
        this.solveResult.penalties == 2000 ? this.solveResult.penalties -= 2000 : this.solveResult.penalties += 2000;
      }
    },
  ...
  computed: {
    isInspectionPenalty() {
      return this.inspectionTimeElapsed > 15000 && this.inspectionTimeElapsed <= 17000;
    },

    isInspectionDnf() {
      return (this.isInspecting && this.timeElapsed > 17000) || this.inspectionTimeElapsed > 17000;
    },
  ...
  watch: {
    ...
    'inputs.manualTime': function(val) {
      ...
      if(val.length > 5) {
        ms = parseInt(val.substring(val.length - 5)) + parseInt(val.substring(0, val.length - 5))*60*1000;
      } else {
        ms = parseInt(val);
      }
