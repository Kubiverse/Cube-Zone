  ...
  <div>
    ...
    >
      ...
      <template v-slot:item.actions="{ item }">
        <v-icon small @click.stop="enterRoom(item)" title="Enter Room">mdi-location-enter</v-icon>
        <v-icon v-if="hasEditPermissions" small @click.stop="openUntrackRoomSolvesDialog(item)" title="Untrack Room Solves" color="error">mdi-alarm-off</v-icon>
        <template v-if="allowEdit && isItemCreator(item)">
          <v-icon small @click.stop="openEditRoomDialog(item)">mdi-pencil</v-icon>
          <v-icon small @click.stop="openDeleteRoomDialog(item)">mdi-delete</v-icon>
        </template>
      </template>
    ...
    <UntrackRoomSolvesDialog :status="dialogs.untrackRoomSolves" :selectedItem="selectedItem" @close="dialogs.untrackRoomSolves = false"></UntrackRoomSolvesDialog>
...
export default {
  ...
  computed: {
    hasEditPermissions() {
      return this.cuber && this.$store.getters["auth/user"]?.id === this.cuber?.id;
    }
  }
}
</script>