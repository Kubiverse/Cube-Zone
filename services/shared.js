...
export default {
  ...
  handleError: function(err, root = null) {
    if (root) {
      if (err.response && err.response.data.info) {
        ...
        this.generateSnackbar(root, this.sanitizeErrorMessage(err.message), "error");
      }
    }
    console.log(err);
  },
  ...
  generateSolveString(solve, currentRound = false) {
    if(!solve) return "";
    return solve.is_dnf ? "DNF" : (solve.state == "FINISHED" ? this.generateTimeString(solve.result) + (solve.penalties ? "+" : "") : (currentRound ? solveStateTextMap[solve.state] : ""));
  },

  generateAccumulatedResultString(accumulated) {
    if(!accumulated) return "N/A";
    return accumulated.is_dnf ? "DNF" : this.generateTimeString(accumulated.result.toFixed());
  },
  ...
  generateTimeString(ms) {
    ...
    if(wholeSeconds < 10 && wholeMinutes) {
      str = wholeMinutes + ":" + (wholeSeconds+"").padStart(2, "0") + "." + str;
    } else {
  ...
  goToWcaProfile(wca_id) {
    if(process.client) {
      window.open("https://www.worldcubeassociation.org/persons/" + wca_id, "_blank");
    }
  },
